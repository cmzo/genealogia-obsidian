...
function syncFromObsidian() {
  ...
  const sourcePath = path.join(obsidianVaultPath, 'Arbol Genealogico');
  const destPath = path.join(process.cwd(), 'content/Arbol Genealogico');
  
  // Verificar si existe la bóveda
  if (!fs.existsSync(sourcePath)) {
    logError(`Carpeta no encontrada: ${sourcePath}`);
    return false;
  }
  
  logInfo(`Sincronizando: ${sourcePath} → ${destPath}`);
